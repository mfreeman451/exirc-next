defmodule ExIRC.Client do
  ...
  def handle_info({:tcp_closed, _socket}, %ClientState{server: server, port: port} = state) do
    ExIRC.Logger.info("Connection to #{server}:#{port} closed!")
    send_event(:disconnected, state)
  ...
  # Handles the client's socket connection 'closed' event (SSL)
  ...
  def handle_data(
    ...
    {:noreply,
     %{state | whois_buffers: put_in(state.whois_buffers, [nick, :certfp], fingerprint)}}
  end

  def handle_data(%ExIRC.Message{cmd: @rpl_whoisregnick, args: [_sender, nick, _message]}, state) do
    {:noreply,
     %{state | whois_buffers: put_in(state.whois_buffers, [nick, :registered_nick?], true)}}
  end
  ...
  def handle_data(
    ...
    {:noreply,
     %{state | whois_buffers: put_in(state.whois_buffers, [nick, :account_name], account_name)}}
  end
  ...
  def handle_data(
    ...
    if String.contains?(message, "identify") do
      ...
      if state.debug?, do: debug("* #{message} from #{from}")
