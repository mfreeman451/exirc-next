defmodule Example.Bot do
  ...
  def handle_info({:login_failed, :nick_in_use}, config) do
    nick = Enum.map(1..8, fn _x -> Enum.random(~c"abcdefghijklmnopqrstuvwxyz") end)
  ...
  def handle_info({:mentioned, msg, %SenderInfo{nick: nick}, _channel}, config) do
    Logger.warning("#{nick} mentioned you")
